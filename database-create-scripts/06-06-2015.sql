...
CREATE TABLE IF NOT EXISTS `receipts` (
  ...
  `sales_report_id_from_branch` int(11) DEFAULT NULL,
  `created_at_from_branch` datetime DEFAULT NULL,
  `updated_at_from_branch` datetime DEFAULT NULL,
...
-- Dumping structure for table fixnmix.sales_reports
CREATE TABLE IF NOT EXISTS `sales_reports` (
  ...
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `sales_reports_branch_id_to_branches_id` (`branch_id`),
  CONSTRAINT `sales_reports_branch_id_to_branches_id` FOREIGN KEY (`branch_id`) REFERENCES `branches` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table fixnmix.sales_reports: ~0 rows (approximately)
...
CREATE TABLE IF NOT EXISTS `uncleared_items` (
  ...
  CONSTRAINT `uncleared_items_branch_id_to_branches_id` FOREIGN KEY (`branch_id`) REFERENCES `branches` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `uncleared_items_item_id_to_items_id` FOREIGN KEY (`item_id`) REFERENCES `items` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
-- Dumping data for table fixnmix.users: ~1 rows (approximately)
...
INSERT INTO `users` (`id`, `username`, `password`, `user_level_id`, `last_name`, `first_name`, `middle_name`, `created_at`, `updated_at`) VALUES
	(1, 'administrator', 'administrator', 1, 'Felipe', 'Ryan', 'Malicay', '2015-06-06 14:25:40', '2015-06-06 14:25:40');
