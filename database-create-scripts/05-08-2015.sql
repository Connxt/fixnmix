...
CREATE TABLE IF NOT EXISTS `deliveries` (
  ...
  PRIMARY KEY (`id`),
  KEY `deliveries_branch_id_to_branches_id` (`branch_id`),
  CONSTRAINT `deliveries_branch_id_to_branches_id` FOREIGN KEY (`branch_id`) REFERENCES `branches` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=latin1;

-- Dumping data for table fixnmix.deliveries: ~1 rows (approximately)
...
INSERT INTO `items` (`id`, `description`, `quantity`, `price`, `created_at`, `updated_at`) VALUES
	(1, 'yes', '2', 50, '2015-04-30 00:00:00', '2015-04-30 15:45:45'),
...
CREATE TABLE IF NOT EXISTS `items_delivered` (
  ...
  KEY `item_deliveries_delivery_id_to_deliveries_id` (`delivery_id`),
  CONSTRAINT `item_deliveries_delivery_id_to_deliveries_id` FOREIGN KEY (`delivery_id`) REFERENCES `deliveries` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Dumping data for table fixnmix.items_delivered: ~2 rows (approximately)
/*!40000 ALTER TABLE `items_delivered` DISABLE KEYS */;
INSERT INTO `items_delivered` (`id`, `delivery_id`, `item_id`, `quantity`) VALUES
	(10, 11, 1, 20),
	(11, 11, 2, 10);
...
CREATE TABLE IF NOT EXISTS `items_returned` (
  ...
  KEY `item_returns_return_id_to_returns_id` (`return_id`),
  CONSTRAINT `item_returns_item_id_to_items_id` FOREIGN KEY (`item_id`) REFERENCES `items` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `item_returns_return_id_to_returns_id` FOREIGN KEY (`return_id`) REFERENCES `returns` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
